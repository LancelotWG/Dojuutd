...
public class TestInstrumentVisitor extends VisitorSupport{
	List<String> instXPath=new ArrayList<>();
	...
	public List<String> getInstXPath() {
		return instXPath;
	}
	...
	public void visit(Element node) {
		...
		if (node.getName().equals("Path")&&node.getNamespacePrefix().equals("hc")&&
				node.getParent().getParent().getName().equals("Map")&&
				node.getParent().getParent().getNamespacePrefix().equals("hc")) {
			instXPath.add(node.getText());
		}
		
		if (node.getName().equals("Port") && node.getNamespacePrefix().equals("c")) {
			Element elt = node.getParent().getParent().getParent();
			if (elt.getName().equals("InstrumentDescription") && elt.getNamespacePrefix().equals("inst")) {
		...
		if (node.getName().equals("ConnectorPin")&&node.getNamespacePrefix().equals("c")) {
			...
			cPinpath1.add(node.getPath()+"[@pinID='"+node.attributeValue("pinID")+"';connectorID='"+node.attributeValue("connectorID")+"']");
			//portMap.put(arg0.attributeValue("connectorID")+"-"+arg0.attributeValue("pinID"), arg0.getParent().getParent());
		}
		...
		if (node.getName().equals("Pin")&&node.getNamespacePrefix().equals("c")) {
			...
			cPinpath.add(node.getParent().getParent().getPath()+"[@ID='"+node.getParent().getParent().attributeValue("ID")+"']"
					+"/c:Pins/c:Pin[@ID='"+node.attributeValue("ID")+"']");
